/**
 * Generates a structured roadmap based on the user's input goal.
 * Dynamically calculates duration from the goal string and generates
 * the correct number of weekly milestones instead of always returning 4 weeks.
 *
 * Returns:
 * {
 *   monthly: [ { id, title, description } ],
 *   weekly: [ { id, title, description, duration, tasks: [ { id, title, completed } ] } ],
 *   daily:  [ { id, title, completed } ]
 * }
 */

// ─── Duration Extractor ────────────────────────────────────────────────────────
/**
 * Reads the goal string and returns { totalWeeks, estimatedTime }.
 * Priority: days → weeks → months → default 4 weeks.
 */
const extractDuration = (normalizedGoal) => {
  ...
  if (daysMatch) {
    const totalWeeks = Math.ceil(parseInt(daysMatch[1]) / 7);
    return { totalWeeks, estimatedTime: `${totalWeeks} Weeks` };
  }
  if (weeksMatch) {
  ...
  if (monthsMatch) {
    const totalWeeks = parseInt(monthsMatch[1]) * 4;
    return { totalWeeks, estimatedTime: `${totalWeeks} Weeks` };
  }
...
/**
 * Each generator receives totalWeeks and returns an array of weekly objects
 * that spans the full duration — not just a hardcoded 4.
 */
...
const generateGenericWeeks = (totalWeeks, trimmedGoal) => {
  const phases = [
    {
      title: "Research, Strategy & Audit",
      description: `Audit baseline requirements, study existing patterns, and schedule blocks for: "${trimmedGoal}".`,
      tasks: [
        `Search for guides, case studies, and tutorials regarding ${trimmedGoal}`,
    ...
    },
    {
      ...
      tasks: [
        `Double check final details and fix errors relating to: ${trimmedGoal}`,
...
/**
 * Generates one monthly milestone object per 4-week block in the plan.
 */
const generateMonthlyPlan = (totalWeeks, category, trimmedGoal) => {
  ...
  const fallbackFocus = [
    { title: "Research, Strategy & Foundation", desc: `Audit requirements, gather resources, and lay the groundwork for: "${trimmedGoal}".` },
  ...
  const focusList = monthFocusMap[category] || fallbackFocus;
...
export const generateRoadmap = (goal) => {
  ...
  // 1. Extract duration from the goal string — this was the root cause of the bug
  const { totalWeeks, estimatedTime } = extractDuration(normalizedGoal);

  // 2. Determine category and generate the correctly-sized weekly plan
  ...
  // 3. Build monthly milestones to match the full duration
